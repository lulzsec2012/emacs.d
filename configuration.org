...
#+BEGIN_SRC emacs-lisp
  ;; diable company mode when realgud running
  (use-package realgud
    :hook
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun toggle-enable-bash-completion ()
    (interactive)
    (if bash-completion-enabled
...
#+BEGIN_SRC emacs-lisp
  (use-package workgroups2
    ...
    (wg-workgroups-mode-exit-save-behavior 'none)
    (wg-emacs-exit-save-behavior           'none)
    ;; eglot LSP server bugfix
    (wg-dissociate-buffer-on-kill-buffer nil)
    ;; this does not work
    (wg-mode-line-display-on nil)
    :config
    (bind-keys* ("M-s" . wg-switch-to-workgroup))
    ;; override defined in workgroups2, suppress the error.
...
Cleanup function, clangd generate much .pch files, ans consume lots of space.

#+begin_src emacs-lisp
  (defun MattLu/clangd-cleanup ()
    (interactive)
    (mapc (lambda (x) (dired-delete-file x 'always))
          (directory-files "/tmp/" t "\\(.+\\.pch\\)\\|\\(symbolizer-+.+\\)")))

  (defun MattLu/lsp-auto-cleanup ()
    (if (eglot-managed-p)
        (setq-local clangd-cleanup--timer-objects
                    (run-with-timer 0 7200 'MattLu/clangd-cleanup))
      (cancel-timer clangd-cleanup--timer-objects)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    (add-to-list 'eglot-server-programs
                 '((c++-mode c-mode) "clangd" "-background-index=0"))
...
#+BEGIN_SRC emacs-lisp
  (use-package xref
    :commands (xref--show-defs-minibuffer)
    :custom
...
** COMMENT selectrum + marginalia + ctrlf + consult
...
#+begin_src emacs-lisp
  (use-package consult
    :bind (("C-x M-:" . consult-complex-command)
           ...
           ("M-g r" . consult-ripgrep)      ;; or consult-grep, consult-git-grep
