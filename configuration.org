...
#+begin_src elisp
;;; configuration.el --- -*- lexical-binding: t; -*-
#+end_src
...
#+begin_src bash
ALL_PROXY=socks5://vps-us:1055/
HTTP_PROXY=http://vps-us:1055/
http_proxy=http://vps-us:1055/
#+end_src
...
Only split horizontally when each resulting window can fit 80 columns of code
plus 10 columns reserved for line numbers, fringes, and padding.
...
#+begin_src emacs-lisp
  (defun my/ef-themes-custom-faces ()
    (ef-themes-with-colors
      (if (eq (car custom-enabled-themes) 'ef-melissa-light)
          (custom-theme-set-faces
           'user
           `(font-lock-comment-face
             ((t (:inherit italic :foreground "grey"))))
           `(font-lock-comment-delimiter-face
             ((t (:inherit italic :foreground "grey")))))
        (custom-theme-set-faces
         'user
         `(font-lock-comment-face
           ((t (:inherit italic :foreground ,fg-dim))))
         `(font-lock-comment-delimiter-face
           ((t (:inherit italic :foreground ,fg-dim))))))))
...
#+begin_src emacs-lisp
  (defun my/auto-switch-theme-by-time ()
    "Switch to ef-light at 18:00, ef-elea-dark at 06:00."
    (let* ((hour (string-to-number (format-time-string "%H")))
           ;; 06:00-17:59 → ef-elea-dark (day), 18:00-05:59 → ef-light (night)
           (new-theme (if (or (>= hour 18) (< hour 6))
                          'ef-light
                        'ef-elea-dark)))
      (unless (eq (car custom-enabled-themes) new-theme)
        (when (car custom-enabled-themes)
          (disable-theme (car custom-enabled-themes)))
        ;; Clear theme-pool toggle so load-theme doesn't conflict
        (setq my/choosing-theme new-theme)
        (my/load-theme new-theme))))

  ;; Run on startup
  (my/auto-switch-theme-by-time)

  ;; Schedule daily switches
  (run-at-time "6:00am" 86400 #'my/auto-switch-theme-by-time)   ;; 06:00 → dark
  (run-at-time "6:00pm" 86400 #'my/auto-switch-theme-by-time)   ;; 18:00 → light
#+end_src
      ...
      (add-hook 'after-make-frame-functions
                (lambda (frame)
                  (with-selected-frame frame
                    (my/load-theme my/choosing-theme)
                    ;; Keep the mode line neat by removing underline styling.
...
#+BEGIN_SRC emacs-lisp
  (cond
   ((eq system-type 'gnu/linux) (setq my/default-font "Ubuntu Mono"))
   ((eq system-type 'darwin)    (setq my/default-font "Iosevka SS09"))
   (t                          (setq my/default-font "Iosevka SS09")))
...
#+BEGIN_SRC emacs-lisp
  (use-package git-gutter
    ...
    :hook
    (git-gutter-mode . (lambda () (setq-local left-margin-width 1)))
...
((nil . ((compilation-environment
          ...
          ("PATH=/usr/local/cuda/bin"
           ;; Installing libgccjit-13-dev may pull in gcc-14, which pollutes the GCC
           ;; toolchain that Clang auto-discovers. Pin Clang to the GCC 13 install dir.
...
#+BEGIN_SRC emacs-lisp
  (use-package flymake
    ...
    ;; keep the window layout stable when buffers resize.
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    ("C-x g" . magit-status)
    :config
    (require 'magit-extras)        ;; 预加载，避免上下键触发 autoload 失败
...
#+begin_src emacs-lisp
  (defcustom my/llvm-objdump-program "sipu-llvm-objdump"
    "Path to the llvm-objdump executable used for disassembly."
    :type 'file
    :group 'tools)

  (defcustom my/llvm-objdump-args '("--mattr=+m,+c,+f,+a,+xsiorigin" "-zldCS")
  ...
  (defun my/objdump (file)
    ...
    (let* ((program (or (executable-find my/llvm-objdump-program)
                        (user-error "Cannot find %s in exec-path" my/llvm-objdump-program)))
...
Generate device-only LLIR for the specified kernel source.
#+begin_src makefile
$(CC) -x sipu -std=c++20 ${CFLAGS} ${INC_PATHS} --offload-device-only -emit-llvm -S $$< -o out.ll
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    :custom
    (eglot-autoshutdown t)         ;; 不再编辑时自动杀掉 LSP 进程，释放内存
    ...
    (my/test-then-add-eglot '(c-or-c++-mode c-mode c++-mode c-or-c++-ts-mode c++-ts-mode c-ts-mode) "clangd")
    ;; (my/test-then-add-eglot '(python-mode python-ts-mode) "pyright-langserver" "--stdio")
    (my/test-then-add-eglot '(python-mode python-ts-mode) "ty" "server")
  ...
  ;; --- LRU LSP 进程淘汰策略 ---
  ;; 保持最多 `my/eglot-max-servers` 个 LSP 常驻，
  ;; 切换 buffer 时杀掉最久未用的那个，避免内存爆炸。
  ;; 被杀掉的 LSP 在下一次切回时自动重启。
  (defvar my/eglot-max-servers 3
    "最多同时保留 N 个 LSP 进程，超出的按 LRU 淘汰。")

  (defvar my/eglot-server-activity (make-hash-table :test #'equal)
    "LSP 进程最后使用时间。key=(project-dir . languageId), value=float-time。")

  (defvar my/eglot-evicted-projects nil
    "已被 LRU 淘汰的 LSP 对应的 project 列表，用于切回时自动重启。")

  (defun my/eglot--server-key (server)
    "生成 LSP 进程的唯一标识。"
    (cons (expand-file-name (eglot--project server))
          (eglot--languageId server)))

  (defun my/eglot--current-server-key ()
    "获取当前 buffer 所属 LSP 的 key。"
    (when-let* ((server (ignore-errors (eglot-current-server))))
      (my/eglot--server-key server)))

  (defun my/eglot-record-activity ()
    "记录当前 buffer 的 LSP 进程最后使用时间。"
    (when-let* ((key (my/eglot--current-server-key)))
      (puthash key (float-time) my/eglot-server-activity)))

  (defun my/eglot--all-servers ()
    "获取所有活跃的 LSP 进程列表。"
    (cl-loop for ss being the hash-values of eglot--servers-by-project
             append ss))

  (defun my/eglot-evict-lru ()
    "LSP 进程数超过上限时，杀掉最久未用的那个（保留当前的）。"
    (let* ((servers (my/eglot--all-servers))
           (count (length servers)))
      (when (> count my/eglot-max-servers)
        (let* ((current-key (my/eglot--current-server-key))
               (lru (car (sort
                          (cl-remove-if
                           (lambda (s) (equal (my/eglot--server-key s) current-key))
                           servers)
                          (lambda (a b)
                            (< (gethash (my/eglot--server-key a)
                                        my/eglot-server-activity 0)
                               (gethash (my/eglot--server-key b)
                                        my/eglot-server-activity 0))))))
               (evict-key (and lru (my/eglot--server-key lru))))
          (when evict-key
            (push evict-key my/eglot-evicted-projects)
            (remhash evict-key my/eglot-server-activity)
            (with-demoted-errors "[LRU eglot evict] %s"
              (eglot-shutdown lru nil nil t)))))))

  (defun my/eglot-reconnect-evicted ()
    "如果当前 buffer 的 LSP 之前被 LRU 淘汰了，自动重启。"
    (when-let* ((key (my/eglot--current-server-key))
                ((member key my/eglot-evicted-projects)))
      (setq my/eglot-evicted-projects
            (delete key my/eglot-evicted-projects))
      (eglot-ensure)))

  (defun my/eglot--on-buffer-switch ()
    "buffer 切换时的统一处理：记录活动 + 淘汰 + 重连。"
    (my/eglot-record-activity)
    (my/eglot-evict-lru)
    (my/eglot-reconnect-evicted))

  ;; 注册到 buffer 切换钩子
  (add-hook 'buffer-list-update-hook #'my/eglot--on-buffer-switch)
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package vterm
    ...
    ;; set .dir-locals
    ;; (eval . (setq vterm-environment compilation-environment))
    (add-to-list 'safe-local-eval-forms
                 '(setq vterm-environment compilation-environment))
...
;; macOS: right Option key → Super (avoid accidental Meta presses)
...
#+begin_src emacs-lisp
  ;; Right-click yanks (pastes) at click position instead of showing menu
  ;; Must bind [down-mouse-3] because context menu fires on press, not release
  (when (bound-and-true-p context-menu-mode)
    (context-menu-mode -1))
  (global-set-key [down-mouse-3] 'mouse-yank-at-click)
#+end_src

** Shell completion: fix dot-file TAB in cd/path

Shell-mode 的 pcomplete 词边界检测将 `.` 视为标点符号，导致 `.emacs` TAB 补全时只传 `"emacs"` 给补全表而找不到 `.emacs.d`。
改用 comint-filename-completion 处理文件路径，文件不匹配时回退到 pcomplete。

#+begin_src emacs-lisp
  (defun my-shell-capf ()
    "Shell completion: file-name completion first, pcomplete fallback.
  Fixes pcomplete's dot-file bug (treats `.` as punctuation boundary)."
    (or (comint-filename-completion)
        (pcomplete-completions-at-point)))

  (add-hook 'shell-mode-hook
            (defun my-shell-mode-capf-setup ()
              (setq-local completion-at-point-functions '(my-shell-capf))))
#+end_src
...
#+begin_src emacs-lisp
  (defun my/insert-boxed-comment (title)
    "Insert a boxed comment with TITLE like LLVM style."
    (interactive "sTitle: ")
    (let* ((bar "//===----------------------------------------------------------------------===//")
           (line (format "// %-70s" title)))
      (insert bar "\n" line "\n" bar "\n")))
#+end_src
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (unless (my/internet-up-p)
      (let* ((corporate-users '("man.lu" "mattlu" "luman" "mzwqe"))
             (proxy-url (if (member user-login-name corporate-users)
                            "socks5h://vps-us:1055"
                          (format "socks5h://%s" my/socks-proxy))))
        (setq-default gptel-proxy proxy-url)))
    ...
    (setq gptel--openai
          (gptel-make-openai "ChatGPT"
            ...
            :stream t
            :models '(gpt-5.5)
            :request-params '(:reasoning (:effort "high") :text (:verbosity "low"))))

    (setq gptel--openai--cc
          (gptel-make-openai "ChatGPT-CC"
            :protocol "http"
            :host "cc.siorigin.com:8317"
            :key (lambda (&optional host user)
                   (gptel-api-key-from-auth-source "cc.siorigin.com" "openai"))
            :stream t
            :models '(gpt-5.5)
            :request-params '(:reasoning_effort  "xhigh" :verbosity "low")))
    ...
    (defvar gptel--anthropic
      (gptel-make-anthropic "Claude"
        ...
        :request-params '(:max_tokens 128000 :thinking (:type "adaptive") :output_config (:effort "max"))))
    ...
    (defvar gptel--deepseek
      (gptel-make-deepseek "DeepSeek"
        ...
        :request-params '(:reasoning_effort "max" :output_config (:effort "max"))))
    ...
    (defvar gptel--tenstorrent
      (gptel-make-openai "Tenstorrent"
        :host "console.tenstorrent.com"
...
#+begin_src emacs-lisp
  (defun my/gpt-refactor (gpt-backend gpt-model rewrite-message prompt proxy)
    (let ((gptel-backend gpt-backend)
          (gptel-model gpt-model)
          (gptel-proxy proxy))
      ...
      (gptel-request ;; https://www.youtube.com/watch?v=j2viZGPSydY
          ;; Repeating the prompt can improve LLM performance.
          (format "%s\n\n%s" prompt rewrite-message)
        ...
        (lambda (response info)
          (if (not (stringp response))
              (message "Invalid LLM response (not a string). Status: %s" (plist-get info :status))
...
- =C-c C-c l= toggles live preview in eww (updates on save).
...
#+begin_src emacs-lisp
  (use-package markdown-mode
    ...
    (setq markdown-command "pandoc --standalone --mathjax --from=markdown"
          ...
          ;; Live preview: C-c C-c l opens eww split window
          markdown-split-window-direction 'right
          ;; Auto-delete temp HTML when preview is closed
...
#+begin_src emacs-lisp
  ...
  (use-package emacs
    ...
    (add-hook 'minibuffer-setup-hook #'cursor-intangible-mode)
     ...
     ;; Prevent *Completions* window from leaving "黑块" after minibuffer exit
     (setq completions-auto-hide t)
     (add-hook 'minibuffer-exit-hook
               (lambda () (delete-windows-on "*Completions*" t))))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package fuz
    ...
    :load-path "~/.emacs.d/third_party/fuz.el/"
    :config
...
#+begin_src emacs-lisp
  (use-package embark
    ...
    (defmacro my/define-embark-gpt-refactor (backend-sym &optional model-sym proxy-url)
      "Create a refactoring command for BACKEND-SYM with optional MODEL-SYM.
       Defaults to the backend's first available model when MODEL-SYM is nil.
       PROXY-URL overrides `gptel-proxy' when specified."
      (let* ((backend (eval backend-sym))
             (backend-name (gptel-backend-name backend))
             (model (or model-sym (car (gptel-backend-models backend))))
             (proxy (or proxy-url gptel-proxy))
             (fn (intern (format "%s-%s-refactor" backend-name (symbol-name model)))))
        `(defun ,fn (prompt)
           (my/gpt-refactor ,backend-sym ',model my/gpt-refactor-prompt prompt ,proxy))))

    (define-key embark-file-map     (kbd "o") (my/embark-ace-action find-file))
    (define-key embark-buffer-map   (kbd "o") (my/embark-ace-action switch-to-buffer))
    (define-key embark-bookmark-map (kbd "o") (my/embark-ace-action bookmark-jump))

    (defvar-keymap gptel-model-map
      :doc "Prefix: select an model for refactoring"
      "i" (my/define-embark-gpt-refactor gptel--openrouter ~anthropic/claude-opus-latest)
      "u" (my/define-embark-gpt-refactor gptel--openrouter ~openai/gpt-latest)
      "y" (my/define-embark-gpt-refactor gptel--openrouter ~google/gemini-pro-latest)
      "o" (my/define-embark-gpt-refactor gptel--anthropic claude-opus-4-7 "")
      "p" (my/define-embark-gpt-refactor gptel--openai--cc gpt-5.5 "")
      "g" (my/define-embark-gpt-refactor gptel--gemini gemini-3.1-pro-preview)
      "d" (my/define-embark-gpt-refactor gptel--deepseek deepseek-v4-pro ""))

    (fset 'gptel-model-map gptel-model-map)
    (keymap-set embark-region-map "g" 'gptel-model-map)
...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    (corfu-auto nil)               ;; 改为手动 TAB 触发，减少每键 LSP IPC
    (global-corfu-modes '((not shell-mode) t)) ;; shell 用自带补全，避免终端弹窗残留
    (corfu-quit-no-match t)      ;; Quit if there is no match.
    (corfu-preview-current nil)
    (corfu-on-exact-match 'quit)   ;; 匹配后自动关闭弹窗
  ...
  ;; Shell mode completion — company-shell for bash-aware completions,
  ;; bridged via cape so corfu renders the popup.
  (use-package company-shell
    :ensure t
    :after cape
    :hook (shell-mode . my/shell-completion-setup)
    :config
    (defun my/shell-completion-setup ()
      "Configure `completion-at-point-functions' in shell-mode.
Uses `company-shell' (commands, builtins, aliases) via the cape bridge,
plus `cape-file' for paths and `cape-dabbrev' for buffer words."
      (setq-local completion-at-point-functions
                  (list (cape-company-to-capf #'company-shell)
                        #'cape-file
                        #'cape-dabbrev))))
...
#+begin_src emacs-lisp
  (use-package dired-rsync
    :defer nil
    :config
    (add-to-list 'mode-line-misc-info 'dired-rsync-modeline-status t)
    :bind
    (:map  dired-mode-map
           ("C-c C-r" . dired-rsync)
...
#+begin_src emacs-lisp
  (setq-default
   ...
   ;; Don't be stingy with history; default is to keep just 16 columns
   search-ring-max 100
...
#+begin_src emacs-lisp
  (setq gc-cons-threshold (* 64 1024 1024))       ;; 64MB，避免单次 GC 冻结太久
  (setq read-process-output-max (* 1 1024 1024))  ;; 1MB，过大阻塞主线程
#+end_src
...
* OpenCode Emacs interface
** sczi/opencode.el — Main OpenCode UI

#+BEGIN_SRC emacs-lisp
(use-package opencode
  :ensure nil                      ;; VC package, not in MELPA
  :load-path "elpa/opencode"
  :demand t
  :commands (opencode opencode-connect opencode-new-session
              opencode-add-file-dwim opencode-add-buffer-dwim
              opencode-add-region opencode-select-project
              opencode-select-open-session)
  :custom
  ;; Remote server connection (via SSH port forwarding)
  (opencode-host "localhost")
  ;; Default port 4096, avoid 9090 conflicts
  (opencode-port 4096)
   ;; Auto-start a headless opencode server if one isn't running
  (opencode-auto-start-server t)
  :config
  ;; Strip inherited OPENCODE_PID / OPENCODE_PROCESS_ROLE from process env
  ;; before starting the server, otherwise the new server registers as a
  ;; worker of an existing TUI session and returns HTTP 502.
  (setq opencode-command
        "env -u OPENCODE_PID -u OPENCODE_PROCESS_ROLE opencode")
  :bind
  ("C-c O o" . opencode)
  ("C-c O c" . opencode-connect)
  ("C-c O n" . opencode-new-session)
  ("C-c O f" . opencode-add-file-dwim)
  ("C-c O b" . opencode-add-buffer-dwim)
  ("C-c O r" . opencode-add-region)
  ("C-c O s" . opencode-select-open-session)
  ("C-c O p" . opencode-select-project))
#+END_SRC

** emacs-mcp-server — Let OpenCode agent control Emacs
...
Key binding guide (C-c O prefix):
  C-c O o  opencode                 — Open sessions buffer
  C-c O c  opencode-connect         — Connect to remote server
  C-c O n  opencode-new-session     — Create new session
  C-c O f  opencode-add-file-dwim   — Add file to context
  C-c O b  opencode-add-buffer-dwim — Add buffer to context
  C-c O r  opencode-add-region      — Add region to context
  C-c O s  opencode-select-open-session — Pick session
   C-c O p  opencode-select-project  — Change project

** Align keybindings with VS Code

Add keybindings that match VS Code defaults, based on ~/keybinding-comparison.md analysis.
These are safe additions with no conflicts against existing bindings.

#+begin_src emacs-lisp
  ;; C-<home> / C-<end> — beginning/end of file (VS Code Ctrl+Home/End)
  (global-set-key (kbd "C-<home>") #'beginning-of-buffer)
  (global-set-key (kbd "C-<end>")  #'end-of-buffer)

  ;; M-<up>/<down> — move current line up/down (VS Code Alt+↑/↓)
  (defun my/move-line-up ()
    "Move current line up by one.  Like VS Code Alt+Up."
    (interactive)
    (if (bobp)
  ...
  (defun my/move-line-down ()
    "Move current line down by one.  Like VS Code Alt+Down."
    (interactive)
  ...
  ;; C-<return> — insert blank line below (VS Code Ctrl+Enter)
  (global-set-key (kbd "C-<return>") #'crux-smart-open-line)

  ;; C-S-<return> — insert blank line above (VS Code Ctrl+Shift+Enter)
  (defun my/smart-open-line-above ()
    "Insert a new line above current and indent.  Like VS Code Ctrl+Shift+Enter."
    (interactive)
    (move-beginning-of-line 1)
    (newline)
    (forward-line -1)
    (indent-according-to-mode))
  (global-set-key (kbd "C-S-<return>") #'my/smart-open-line-above)

  ;; C-? — redo (VS Code Ctrl+Y).  C-? = C-S-/, terminal-safe in CSI u mode.
