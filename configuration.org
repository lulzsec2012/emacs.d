...
#+begin_src emacs-lisp
  (setq MattLu/YCMD-path '("/usr/local/lib/ycmd/ycmd/"
                           (file-truename "~/.emacs.d/ycmd/ycmd/")))
  (defun MattLu/YCMD-set-path (path-list)
...
#+BEGIN_SRC emacs-lisp
  (use-package ycmd
    ...
    (MattLu/YCMD-set-path MattLu/YCMD-path))
...
#+BEGIN_SRC emacs-lisp
  (use-package virtualenvwrapper
    ...
    (cond ((and (eq system-type 'gnu/linux)
                (file-directory-p "~/python/ufw/"))
           (venv-workon "ufw"))
          ((and (eq system-type 'darwin)
                (file-directory-p "~/venvPy/py3/"))
           (venv-workon "py3"))))
#+END_SRC
...
=BUG=
fast-scroll may enable flyspell although we disable flyspell by defaults.

#+BEGIN_SRC emacs-lisp
  (use-package fast-scroll
    :hook
    ((fast-scroll-start . turn-off-flyspell)
     (fast-scroll-end . turn-on-flyspell)
     (flyspell-mode . fast-scroll-mode))
    :config
    (fast-scroll-config)
    (setq fast-scroll-throttle 0.5))
#+END_SRC

** COMMENT jump back and forth
...
#+begin_src emacs-lisp
  (use-package back-button
    :bind (:map back-button-mode-map
                ...
                ("<C-f6>" . hydra-back-and-forth/back-button-global-forward))
    :init
    (back-button-mode 1)
