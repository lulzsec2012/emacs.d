...
#+begin_src emacs-lisp
  ...
  (defun my/new-socks5-proxy ()
    (let* ((command-output (shell-command-to-string (expand-file-name "~/.ssh/vpn-config.py")))
           (command (split-string-and-unquote command-output))
           (program (car command))
           (args (cdr command)))
      (apply #'start-process "socks5-proxy"
             (format "*socks5-proxy-%s*" "vpn")
             program args)))
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    :bind (:map eglot-mode-map
                ...
                ("C-c f i" . eglot-inlay-hints-mode))
    :custom
...
#+begin_src emacs-lisp
  (use-package vterm
    ...
    (mapcar
     ...
     env-var)
    (setq my/vterm-cmd (mapconcat (lambda (x) (format "vterm_read_%s\n" x)) env-var ""))
    (defun my/setup-vterm-python-env ()
      ;; get environment variable form vterm and set to emacs
      (interactive)
      (setq python-shell-process-environment nil)
      (vterm-send-string my/vterm-cmd))
    :bind
    (:map vterm-mode-map
          ("C-c x" . my/setup-vterm-python-env)
          ("M-f" . (lambda ()
                     (interactive)
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (defun +get-api-key (path)
      (string-trim-right (f-read-text path)))

    (gptel-make-openai "ChatGPT"
      :key (+get-api-key "~/.ssh/gpt-api-key")
      :stream t
      :models '("gpt-4o" "gpt-4-turbo"))

    (defvar gptel--gemini
      (gptel-make-gemini "Gemini"
        :key (+get-api-key "~/.ssh/gemini-api-key")
        :stream t
        :models '("gemini-1.5-pro-latest")))

    (defvar gptel--anthropic
      (gptel-make-anthropic "Claude"
        :key (+get-api-key "~/.ssh/claude-api-key")
        :stream t
